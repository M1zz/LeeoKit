...
4. **짝은 맨 앞에 선다.** 한 짝으로 만든 앱(아이폰 앱 ↔ 맥 앱)은 계약에 적어 두면 이야기보다도
   먼저, "이 앱과 짝인 앱"으로 따로 선다. 지금 기기에서 못 받는 앱이어도 그렇다.

```swift
static let familyID: String? = "rainbow-ios"
static let familyFeatured = ["rainbow-mac"]   // 권할 순서대로. 없는 id·자기 자신은 조용히 빠진다
```
